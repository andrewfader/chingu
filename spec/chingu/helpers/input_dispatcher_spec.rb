...
describe Chingu::Helpers::InputDispatcher do
  ...
  {"button_down" => :a, "button_up" => :released_a}.each_pair do |event, key|
    describe "#dispatch_#{event}" do
      it "should dispatch key event if key is handled" do
        ...
        allow(@client).to receive(:input).with(no_args).and_return({ key => [@client.method(:handler)] })
        @subject.send("dispatch_#{event}", Gosu::KbA, @client)
      end

      it "should not dispatch key event if key is not handled" do
        allow(@client).to receive(:input).with(no_args).and_return({})
        @subject.send("dispatch_#{event}", Gosu::KbA, @client)
      end
    end
  end
  ...
  describe "#dispatch_input_for" do
    before :each do
      ...
      allow($window).to receive(:button_down?).and_return(false)
    end
    ...
    it "should dispatch if a key is being held" do
      @client.should_receive(:handler).with(no_args)
      allow($window).to receive(:button_down?).with(Gosu::KbA).and_return(true)
      allow(@client).to receive(:input).with(no_args).and_return({:holding_a => [@client.method(:handler)]})
      @subject.dispatch_input_for(@client)
    end

    it "should do nothing if a key is not held" do
      allow(@client).to receive(:input).with(no_args).and_return({:holding_a => [lambda { raise "Shouldn't handle input!"}]})
      @subject.dispatch_input_for(@client)
    end
  end
