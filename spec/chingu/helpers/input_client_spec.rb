...
module Chingu
  describe Helpers::InputClient do
    before :each do
      ...
      allow(@subject).to receive(:handler1).and_return(nil)
      @handler1 = @subject.method :handler1
      allow(@subject).to receive(:handler2).and_return(nil)
    ...
    describe "#holding_all?" do
      ...
      it "should be false if some of those keys are not being held down" do
        allow($window).to receive(:button_down?).with(Gosu::KbSpace).and_return(true)
        @subject.holding_all?(:space, :a).should be_falsey
      end
    end
